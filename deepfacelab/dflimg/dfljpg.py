...
from deepfacelab.core.imagelib import seg_ie_polys
from deepfacelab.core.cv2ex import cv2_imread
from deepfacelab.core.structex import struct_unpack
...
class DFLJPG:
    ...
    def get_seg_ie_polys(self):
        ...
        if d is not None:
            d = seg_ie_polys.SegIEPolys.load(d)
        else:
            d = seg_ie_polys.SegIEPolys()

        return d

    def set_seg_ie_polys(self, seg_ie_polys):
        if seg_ie_polys is not None:
            if not isinstance(seg_ie_polys, seg_ie_polys.SegIEPolys):
