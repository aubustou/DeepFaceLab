...
from deepfacelab.core import randomex
...
def mls_rigid_deformation(vy, vx, src_pts, dst_pts, alpha=1.0, eps=1e-8):
    ...
    reshaped_mul_right = np.concatenate(
        (
            vpstar[:, None, ...],
    ...
    for i in range(ctrls):
        ...
        temp += np.matmul(
            qhat.reshape(1, 2, grow, gcol).transpose(2, 3, 0, 1),
            np.matmul(
                (
                    w[None, i : i + 1, ...]
                    * np.concatenate(
                        (
                            phat.reshape(1, 2, grow, gcol),
                            np.concatenate((phat[None, 1:2], -phat[None, 0:1]), 1),
                        ),
                        0,
                    )
                ).transpose(2, 3, 0, 1),
    ...
    transformers = (
        np.true_divide(temp, normed_temp, out=np.zeros_like(temp), where=~nan_mask)
        * normed_vpstar
        + qstar
    )
...
def gen_warp_params(
    ...
    mapx[1:-1, 1:-1] = mapx[1:-1, 1:-1] + randomex.random_normal(
        size=(cell_count - 2, cell_count - 2), rnd_state=warp_rnd_state
    ) * (cell_size * 0.24)
    ...
    rot = float(rotation)
    sc = float(scale)
    logger.info("w: %s, rotation: %s, scale: %s", w, rotation, scale)
    random_transform_mat = cv2.getRotationMatrix2D((cx, cy), rot, sc)
