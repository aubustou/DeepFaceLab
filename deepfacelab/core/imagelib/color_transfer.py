...
def color_transfer_sot(
    src, trg, steps=10, batch_size=5, reg_sigmaXY=16.0, reg_sigmaV=5.0
):
    """
...
def _scale_array(arr, clip=True):
    ...
    if mn < scale_range[0] or mx > scale_range[1]:
        return (scale_range[1] - scale_range[0]) * (arr - mn) / (mx - mn) + scale_range[
            0
        ]
...
def color_transfer_mix(img_src, img_trg):
    ...
    rct_light = np.clip(
        linear_color_transfer(
            img_src_lab[..., 0:1].astype(np.float32) / 255.0,
            img_trg_lab[..., 0:1].astype(np.float32) / 255.0,
        )[..., 0]
        * 255.0,
        0,
        255,
    ...
    img_rct = color_transfer_sot(
        img_src_lab.astype(np.float32), img_trg_lab.astype(np.float32)
    )
