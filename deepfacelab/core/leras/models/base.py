...
from deepfacelab.core.leras.layers.saveable import Saveable
from deepfacelab.core.leras.layers.base import LayerBase
...
class ModelBase(Saveable):
    ...
    def _build_sub(self, layer, name):
        ...
        elif isinstance(layer, LayerBase) or isinstance(layer, ModelBase):
    ...
    def xor_list(self, lst1, lst2):
        return [
            value for value in lst1 + lst2 if (value not in lst1) or (value not in lst2)
        ]
    ...
    def summary(self):
        ...
        for layer in layers:
            if len(str(layer)) > max_len_str:
                max_len_str = len(str(layer))
            layers_names += [str(layer).capitalize()]

        # Get params for each layer
        layers_params = [
            int(np.sum(np.prod(w.shape) for w in layer.get_weights())) for layer in layers
        ]
        ...
        for i in range(len(delim_str) - len_model_name_str):
            model_name_str += (
                " " if i != (len(delim_str) - len_model_name_str - 2) else " |"
            )
        ...
        for i in range(len(delim_str) - len_total_params_str):
            total_params_str += (
                " " if i != (len(delim_str) - len_total_params_str - 2) else " |"
            )
