...
class UNetPatchDiscriminator(ModelBase):
    """
    ...
    def on_build(self, patch_size, in_ch, base_ch=16, use_fp16=False):
        ...
        self.in_conv = Conv2D(
            in_ch, level_chs[-1], kernel_size=1, padding="VALID", dtype=conv_dtype
        )
        ...
        self.out_conv = Conv2D(
            level_chs[-1] * 2, 1, kernel_size=1, padding="VALID", dtype=conv_dtype
        )

        self.center_out = Conv2D(
            level_chs[len(layers) - 1],
            1,
            kernel_size=1,
            padding="VALID",
            dtype=conv_dtype,
