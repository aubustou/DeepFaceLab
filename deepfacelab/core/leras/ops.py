...
from tensorflow.python.ops import array_ops, random_ops, gradients
...
class NNMixin:
        ...
        if len(tuples) != 0:
            ...
            with tf.device("/CPU:0"):
                ...
                for x, value in tuples:
                    if isinstance(value, tf.Operation) or isinstance(
                        value, tf.Variable
                    ):
                        ...
                        assign_op = tf.assign(x, assign_placeholder)
    ...
    def init_weights(weights):
        ...
        if len(ca_tuples) != 0:
            nn.batch_set_value(
                [*zip(ca_tuples_w, nn.initializers.ca.generate_batch(ca_tuples))]
            )
    ...
    def gradients(loss, vars):
        ...
        for g, v in gv:
            if g is None:
                raise Exception(
                    f"Variable {v.name} is declared as trainable, but no tensors flow through it."
                )
        return gv
    ...
    def average_tensor_list(tensors_list, tf_device_string=None):
        ...
        result = tf.reduce_mean(
            tf.concat([tf.expand_dims(t, 0) for t in tensors_list], 0), 0
        )
        if e is not None:
            e.__exit__(None, None, None)
        return result
    ...
    def gelu(x):
        cdf = 0.5 * (
            1.0 + tf.nn.tanh((np.sqrt(2 / np.pi) * (x + 0.044715 * tf.pow(x, 3))))
        )
        return x * cdf
    ...
    def upsample2d(x, size=2):
        if nn.data_format == "NCHW":
            ...
            x = tf.image.resize_nearest_neighbor(
                x, (x.shape[1] * size, x.shape[2] * size)
            )
    ...
    def gaussian_blur(input, radius=2.0):
        ...
        x = tf.nn.depthwise_conv2d(
            x, k, strides=[1, 1, 1, 1], padding="VALID", data_format=nn.data_format
        )
        return x

    @staticmethod
    def style_loss(
        target, style, gaussian_blur_radius=0.0, loss_weight=1.0, step_size=1
    ):
        from deepfacelab.core.leras.nn import nn
        def sd(content, style, loss_weight):
            ...
            c_mean, c_var = tf.nn.moments(
                content, axes=nn.conv2d_spatial_axes, keep_dims=True
            )
            s_mean, s_var = tf.nn.moments(
                style, axes=nn.conv2d_spatial_axes, keep_dims=True
            )
    ...
    def total_variation_mse(images):
        ...
        tot_var = tf.reduce_sum(tf.square(pixel_dif1), axis=[1, 2, 3]) + tf.reduce_sum(
            tf.square(pixel_dif2), axis=[1, 2, 3]
        )
        return tot_var
    ...
    def pixel_norm(x, axes):
        ...
        return x * tf.rsqrt(
            tf.reduce_mean(tf.square(x), axis=axes, keepdims=True) + 1e-06
        )
