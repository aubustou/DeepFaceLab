...
from deepfacelab.core.leras.layers.dense import Dense
from deepfacelab.core.leras.models.base import ModelBase

import tensorflow.compat.v1 as tf
from .base import ArchiBase
        ...
        if mod is None:
                ...
                def on_build(self):
                    if "t" in opts:
                        ...
                        self.down3 = Downscale(
                            self.e_ch * 2, self.e_ch * 4, kernel_size=5
                        )
                        self.down4 = Downscale(
                            self.e_ch * 4, self.e_ch * 8, kernel_size=5
                        )
                        self.down5 = Downscale(
                            self.e_ch * 8, self.e_ch * 8, kernel_size=5
                        )
            ...
            class Inter(ModelBase):
                ...
                def forward(self, inp):
                    ...
                    x = self.dense2(x)
                    x = nn.reshape_4D(
                        x, lowest_dense_res, lowest_dense_res, self.ae_out_ch
                    )
                ...
                def on_build(self, in_ch, d_ch, d_mask_ch):
                    if "t" not in opts:
                        ...
                        self.upscalem1 = Upscale(
                            d_mask_ch * 8, d_mask_ch * 4, kernel_size=3
                        )
                        self.upscalem2 = Upscale(
                            d_mask_ch * 4, d_mask_ch * 2, kernel_size=3
                        )

                        self.out_conv = Conv2D(
                            d_ch * 2, 3, kernel_size=1, padding="SAME", dtype=conv_dtype
                        )

                        if "d" in opts:
                            self.out_conv1 = Conv2D(
                                d_ch * 2,
                                3,
                                kernel_size=3,
                                padding="SAME",
                            ...
                            self.upscalem3 = Upscale(
                                d_mask_ch * 2, d_mask_ch * 1, kernel_size=3
                            )
                            self.out_convm = Conv2D(
                                d_mask_ch * 1,
                                1,
                                kernel_size=1,
                                padding="SAME",
                        ...
                        self.upscalem0 = Upscale(in_ch, d_mask_ch * 8, kernel_size=3)
                        self.upscalem1 = Upscale(
                            d_mask_ch * 8, d_mask_ch * 8, kernel_size=3
                        )
                        self.upscalem2 = Upscale(
                            d_mask_ch * 8, d_mask_ch * 4, kernel_size=3
                        )
