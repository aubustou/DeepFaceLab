...
class AdaBelief(OptimizerBase):
    ...
    def get_weights(self):
        return (
            [self.iterations]
            + list(self.ms_dict.values())
            + list(self.vs_dict.values())
        )

    def initialize_variables(
        self, trainable_weights, vars_on_cpu=True, lr_dropout_on_cpu=False
    ):
        from deepfacelab.core.leras.nn import nn
    ...
    def get_update_op(self, grads_vars):
        ...
        for i, (g, v) in enumerate(grads_vars):
            ...
            v_diff = (
                -lr * m_t / (tf.sqrt(v_t) + np.finfo(g.dtype.as_numpy_dtype).resolution)
            )
            if self.lr_dropout != 1.0:
