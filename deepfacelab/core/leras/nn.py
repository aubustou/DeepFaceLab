...
from deepfacelab.core.leras.ops import NNMixin

from .device import Devices

if TYPE_CHECKING:
    from tensorflow import Session
    from tensorflow._api.v2.compat.v1 import Module
...
class nn(NNMixin):
            ...
            if len(device_config.devices) != 0:
                if sys.platform[0:3] == "win":
                    ...
                    if all(
                        ...
                        devices_str = "_" + device_config.devices[0].name.replace(
                            " ", "_"
                        )
                    else:
                        devices_str = ""
    ...
    class DeviceConfig:
        ...
        def ask_choose_device(*args, **kwargs):
            return nn.DeviceConfig.GPUIndexes(
                nn.ask_choose_device_idxs(*args, **kwargs)
            )
    ...
    def ask_choose_device_idxs(
        ...
        if suggest_all_gpu:
            ...
            best_device_indexes = [
                device.index
                for device in devices.get_equal_devices(devices.get_best_device())
            ]
        else:
        ...
        while True:
            try:
                if choose_only_one:
                    choosed_idxs = io.input_str(
                        "Which GPU index to choose?", best_device_indexes
                    )
                else:
                    choosed_idxs = io.input_str(
                        "Which GPU indexes to choose?", best_device_indexes
                    )
...
class ArchiMixin:
    ...
    @staticmethod
    def DeepFakeArchi() -> Type[DeepFakeArchi]:
        return DeepFakeArchi
