...
from .mp_shared_list import MPSharedList
...
class IndexHost:
    ...
    def __init__(self, indexes_count, rnd_seed=None):
        ...
        self.thread = threading.Thread(
            target=self.host_thread, args=(indexes_count, rnd_seed)
        )
    ...
    def host_thread(self, indexes_count, rnd_seed):
        rnd_state = (
            np.random.RandomState(rnd_seed) if rnd_seed is not None else np.random
        )
