...
class MPSharedList:
    ...
    def __init__(self, obj_list):
        if obj_list is None:
            ...
            obj_count, table_offset, data_offset, sh_b = MPSharedList.bake_data(
                obj_list
            )
    ...
    def __add__(self, o):
        if isinstance(o, MPSharedList):
            ...
            raise ValueError(
                f"MPSharedList object of class {o.__class__} is not supported for __add__ operator."
            )
    ...
    def __getitem__(self, key):
        ...
        offset_start, offset_end = struct.unpack(
            "<QQ", sh_b[table_offset + key * 8 : table_offset + (key + 2) * 8].tobytes()
        )

        return pickle.loads(
            sh_b[data_offset + offset_start : data_offset + offset_end].tobytes()
        )
    ...
    def bake_data(obj_list):
        ...
        if obj_count != 0:
            ...
            sh_b_view[table_offset : table_offset + table_size] = struct.pack(
                "<" + "Q" * len(offsets), *offsets
            )
