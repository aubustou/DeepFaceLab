...
def get_image_paths(
    dir_path, image_extensions=image_extensions, subdirs=False, return_Path_class=False
):
    ...
    for f in result[:]:
        ...
        if f_stem in result_dup:
            ...
            if verbose_print_func is not None:
                verbose_print_func(
                    "Duplicate filenames are not allowed, skipping: %s" % Path(f).name
                )
            continue
...
def get_file_paths(dir_path: Path | str | PathLike) -> list[Path]:
    if dir_path is None:
        breakpoint()
    dir_path = Path(dir_path)
    ...
    if dir_path.exists():
        return [
            Path(x)
            for x in sorted(
                [x.path for x in list(scandir(str(dir_path))) if x.is_file()]
            )
        ]
    else:
        return []
