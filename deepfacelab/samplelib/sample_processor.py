...
class SampleProcessor:
    ...
    @staticmethod
    def process(
        samples, sample_process_options, output_sample_types, debug, ct_sample=None
    ):
        ...
        for sample in samples:
                ...
                if xseg_mask is not None:
                    if xseg_mask.shape[0] != h or xseg_mask.shape[1] != w:
                        xseg_mask = cv2.resize(
                            xseg_mask, (w, h), interpolation=cv2.INTER_CUBIC
                        )
                        xseg_mask = imagelib.normalize_channels(xseg_mask, 1)
                    return np.clip(xseg_mask, 0, 1)
                else:
                    full_face_mask = landmarks_processor.get_image_hull_mask(
                        sample_bgr.shape,
                        sample_landmarks,
            ...
            def get_eyes_mask():
                eyes_mask = landmarks_processor.get_image_eye_mask(
            ...
            def get_eyes_mouth_mask():
                ...
                mouth_mask = landmarks_processor.get_image_mouth_mask(
                    sample_bgr.shape, sample_landmarks
                )
            ...
            if debug and is_face_sample:
                landmarks_processor.draw_landmarks(
                    sample_bgr, sample_landmarks, (0, 1, 0)
                )
            ...
            for opts in output_sample_types:
                ...
                warp_rnd_state = np.random.RandomState(
                    sample_rnd_seed + warp_rnd_seed_shift
                )
                ...
                if sample_type == SPST.FACE_IMAGE or sample_type == SPST.FACE_MASK:
                    ...
                    if sample_type == SPST.FACE_MASK:
                        if face_mask_type == SPFMT.FULL_FACE:
                            ...
                            img = np.zeros(
                                sample_bgr.shape[0:2] + (1,), dtype=np.float32
                            )
                        ...
                        if sample_face_type == FaceType.MARK_ONLY:
                            ...
                            if face_type != sample_face_type:
                                mat = landmarks_processor.get_transform_mat(
                        ...
                        if random_hsv_shift_amount != 0:
                            ...
                            img_h = (
                                img_h + rnd_state.randint(-h_amount, h_amount + 1)
                            ) % 360
                            img_s = np.clip(
                            ...
                            img = np.clip(
                                cv2.cvtColor(
                                    cv2.merge([img_h, img_s, img_v]), cv2.COLOR_HSV2BGR
                                ),
                                0,
                                1,
                            )
                        ...
                        if channel_type == SPCT.BGR:
                            ...
                            out_sample = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)[
                                ..., None
                            ]
                    ...
                    if nearest_resize_to is not None:
                        out_sample = cv2_resize(
                            out_sample,
                            (nearest_resize_to, nearest_resize_to),
                    ...
                    sample_type == SPST.PITCH_YAW_ROLL
                    or sample_type == SPST.PITCH_YAW_ROLL_SIGMOID
                ):
