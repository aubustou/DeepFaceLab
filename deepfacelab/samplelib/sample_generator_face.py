...
from deepfacelab.core import mplib
from deepfacelab.core.interact import interact as io
from deepfacelab.core.joblib import SubprocessGenerator, ThisThreadGenerator

from . import sample_loader
from .sample import SampleType
from .sample_generator_base import SampleGeneratorBase
...
class SampleGeneratorFace(SampleGeneratorBase):
    def __init__(
        ...
        **kwargs,
        ...
        samples = sample_loader.load(SampleType.FACE, samples_path)
        ...
        if uniform_yaw_distribution:
            samples_pyr = [
                (idx, sample.get_pitch_yaw_roll()) for idx, sample in enumerate(samples)
            ]
            ...
            for g in io.progress_bar_generator(range(grads), "Sort by yaw"):
                ...
                for idx, pyr in samples_pyr:
                    s_yaw = -pyr[1]
                    if (
                        (g == 0 and s_yaw < next_yaw)
                        or (g < grads - 1 and s_yaw >= yaw and s_yaw < next_yaw)
                        or (g == grads - 1 and s_yaw >= yaw)
                    ):
                        yaw_samples += [idx]
        ...
        if random_ct_samples_path is not None:
            ct_samples = sample_loader.load(SampleType.FACE, random_ct_samples_path)
            ...
            self.generators = [
                ThisThreadGenerator(
                    ...
                    (
                        ...
                        ct_samples,
                        (
                            ct_index_host.create_cli()
                            if ct_index_host is not None
                            else None
                        ),
                    ),
                )
