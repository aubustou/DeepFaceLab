...
from deepfacelab.core.imagelib.seg_ie_polys import SegIEPolys
from deepfacelab.facelib import landmarks_processor
from deepfacelab.core.cv2ex import cv2_imread
...
class Sample:
    ...
    def __init__(
        ...
        **kwargs
        ...
        if self.xseg_mask_compressed is None and self.xseg_mask is not None:
            xseg_mask = np.clip(
                imagelib.normalize_channels(xseg_mask, 1) * 255, 0, 255
            ).astype(np.uint8)
        ...
        self.eyebrows_expand_mod = (
            eyebrows_expand_mod if eyebrows_expand_mod is not None else 1.0
        )
    ...
    def get_pitch_yaw_roll(self):
        if self.pitch_yaw_roll is None:
            self.pitch_yaw_roll = landmarks_processor.estimate_pitch_yaw_roll(
    ...
    def load_bgr(self):
        img = (
            cv2_imread(self.filename, loader_func=self.read_raw_file).astype(np.float32)
            / 255.0
        )
        return img
