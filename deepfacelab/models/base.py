...
from deepfacelab.core import pathex, cv2ex
from deepfacelab.core.imagelib import get_text_image, equalize_and_stack_square
from deepfacelab.core.interact import interact as io
from deepfacelab.samplelib.sample_generator_base import SampleGeneratorBase
...
class ModelBase:
    def __init__(
        ...
        **kwargs,
        ...
        self.model_class_name = model_class_name = self.__class__.__name__

        logger.info("Running model %s", model_class_name)
                ...
                while True:
                    ...
                    if len(saved_models_names) != 0:
                                        ...
                                        if len(saved_models_names) != 0:
                                            ...
                                            if name in saved_models_names:
                                                ...
                                                for filepath in pathex.get_paths(
                                                    ...
                                                    model_filename, remain_filename = (
                        ...
                        self.model_name = io.input_str(
                            f"No saved models found. Enter a name of a new model", "new"
                        )
    ...
    def ask_autobackup_hour(self, default_value=0):
        ...
        self.options["autobackup_hour"] = io.input_int(
            ...
            add_info="0..24",
            help_message="Autobackup model files with preview every N hour. Latest backup located in model/<>_autobackups/01",
        )
        ...
        if self.options["write_preview_history"]:
            if io.is_support_windows():
                self.choose_preview_history = io.input_bool(
                    "Choose image for the preview history", False
                )
            elif io.is_colab():
                self.choose_preview_history = io.input_bool(
                    "Randomly choose new image for preview history",
                    False,
    ...
    def ask_target_iter(self, default_value=0):
        ...
        self.options["target_iter"] = max(
            0, io.input_int("Target iteration", default_target_iter)
        )
    ...
    def ask_random_src_flip(self):
        ...
        self.options["random_src_flip"] = io.input_bool(
            ...
            help_message="Random horizontal flip SRC faceset. Covers more angles, but the face may look less naturally.",
        )
    ...
    def ask_random_dst_flip(self):
        ...
        self.options["random_dst_flip"] = io.input_bool(
            ...
            help_message="Random horizontal flip DST faceset. Makes generalization of src->dst better, if src random flip is not enabled.",
        )
        ...
        for i in range(24, 0, -1):
            ...
            if i == 1:
                ...
                for filename in bckp_filename_list:
                    shutil.copy(
                        str(filename), str(idx_backup_path / Path(filename).name)
                    )
    ...
    def debug_one_iter(self):
        ...
        return equalize_and_stack_square(images)
    ...
    def get_summary_text(self):
        ...
        for key in visible_options.keys():
            ...
            ]  # visible_options key/value pairs
        summary_text += [f'=={" "*width_total}==']
    ...
    def get_loss_history_preview(loss_history, iter, w, c):
        ...
        lh_img[last_line_t:last_line_b, 0:w] += get_text_image(
        ...
        while True:
            while not sq.empty():
                ...
                for preview, filepath in plist:
                    ...
                    cv2ex.cv2_imwrite(filepath, img)
